...
const server = http.createServer((req, resp) => {
  let number = Number(req.url.slice(1));
  // edge cases handle agar user ne url me number nhi diya toh validation laga di
  // number agar out of range send kiya itna toh data hi nhi
  //  if condtion ke baad return krenge q ki jab yeh callback function chalega toh js toh line by line code ko read krega toh yeh aage wali liine ko bhi read krega suppose kisi ne abc  daal diya apn ne condtion laga di yeh check kr lega but aage wla code bhi run krega q ki apne ne wha se function ko return hi nhi krawya

  if (isNaN(number) || number <= 0) {
    resp.end("Invalid URL");
    return;
  } else if (number > 100) {
    resp.end("you cannot fetch more then 100 user");
    return;
  }
